...
static bool wc_is_path_separator(char ch) {
    return ch  == '\\' || ch == '/';
}
...
int wc_is_wildcard_pattern(const char* pattern) {
    return wc_get_wildcard_index(pattern) != -1;
}
...
int wc_get_wildcard_index(const char* pattern) {
    ...
    for (int i = 0; i < len; i++) {
        if (wc_is_wildcard_char(pattern[i])) {
            return i;
        }
    }
    return -1;
...
int wc_get_wildcard_path_index(const char* pattern, const char* file_name) {
    ...
    int wildcard_index = wc_get_wildcard_index(pattern);
    if (wildcard_index < 0) {
        return -1;
    }
    return wc_get_wildcard_path_index(wildcard_index, file_name);
}
...
int wc_get_wildcard_path_index(int wildcard_index, const char* file_name) {
    ...
    bool found = false;
    while (i >= 0) {
        if (wc_is_path_separator(file_name[i])) {
            found = true;
            break;
        }
        i--;
    ...
    do {
        ...
        case '*':
            ...
            default:
                ...
                if (wc_match_file(name, pattern + 1))
                    return 1;
