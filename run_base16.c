...
int run_base16_encode_data(const char* data, size_t size) {
    if (!data) {
        return 1;
    }
...
int run_base16_decode_data(const char* data, size_t size) {
    if (!data) {
        return 1;
    }
...
int run_base16_encode_file(const char* file_name) {
    if (!file_name) {
        return 1;
    }
    ...
    retval = run_base16_encode_data(data, size);
    if (data) {
        free(data);
    }
    return retval;
...
int run_base16_decode_file(const char* file_name) {
    if (!file_name) {
        return 1;
    }
    ...
    retval = run_base16_decode_data(data, size);
    if (data) {
        free(data);
    }
    return retval;
...
void usage() {
    fprintf(stderr, "Usage: run-base16 [-D] -s string | file\n");
}
...
int main(int argc, char* argv[]) {
    ...
    if (flag_string) {
        ...
        if (flag_decode) {
            return run_base16_decode_data(data, strlen(data));
        } else {
            return run_base16_encode_data(data, strlen(data));
        }        
    } else {
        if (optind >= argc) {
            usage();
        ...
        if (flag_decode) {
            return run_base16_decode_file(file_name);
        } else {
            return run_base16_encode_file(file_name);
        }        
    }
    return 0;
}   
