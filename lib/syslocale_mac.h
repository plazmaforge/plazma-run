...
static lib_locale_t* loadLocaleMacCurrent() {
  ...
  locale->name = lib_sys_build_locale_name(language, country, encoding);
...
static lib_locale_t* loadLocaleMac(int cat) {
    return loadLocaleMacCurrent(); // Category independed. Why?
}
