...
int _lib_unimap_find_ocode(int* map, size_t len, int ucode) {
...
int lib_unimap_conv_id(int from, int to, char* data, size_t len) {
    ...
    // Get 'from' map
    int* from_map = (int*) _lib_unimap_get_map_id(from);
    if (!from_map) {
        return 1;
    }

    // Get 'to' map
    int* to_map = (int*) _lib_unimap_get_map_id(to);
    if (!to_map) {
        return 1;
    }
    ...
    for (int i = 0; i < len; i++) {
        ...
        int idx = icode - 127 - 1;
        int ucode = from_map[idx];
        int ocode = _lib_unimap_find_ocode(to_map, 128, ucode);
        if (ocode > -1) {
            data[i] = ocode;
        }
    }
